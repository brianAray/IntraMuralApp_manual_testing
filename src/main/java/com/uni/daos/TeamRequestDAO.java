...
public class TeamRequestDAO implements CrudDAO<TeamRequest>{
    ...
    public static TeamRequestDAO getSingleton(HashMap<String, DatabaseStore> data){
    ...
    public TeamRequest save(TeamRequest teamRequest, String devId) {
        DatabaseStore data = null;
        if(database != null){
            data = database.get(devId);
        }
        ArrayList<TeamRequest> teamRequests = data.getTeamRequests();
        teamRequest.setTeamRequestId(teamRequests.size() + 1);
        teamRequests.add(teamRequest);
        data.setTeamRequests(teamRequests);
...
//        }
    ...
    public List<TeamRequest> findAll(String devId) {
        DatabaseStore data = null;
        if(database != null){
            data = database.get(devId);
        }
...
//        }
    ...
    public void update(TeamRequest teamRequest, String devId) {
        DatabaseStore data = null;
        if(database != null){
            data = database.get(devId);
        }
        ArrayList<TeamRequest> teamRequests = data.getTeamRequests();
        teamRequests.set(teamRequest.getTeamRequestId() - 1, teamRequest);
        data.setTeamRequests(teamRequests);
